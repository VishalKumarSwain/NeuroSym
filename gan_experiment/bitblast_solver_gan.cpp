...
// ─────────────────────────── GAN bridge (experimental) ──────────────────────
// Faithful C++ port of gansat/bv_encoder.py's bv_encode/bv_decode_assignment.
// Deliberately replicates a real bug in the original: _BV_OP_INDEX only maps
// ULEQ/UGEQ/SLEQ/SGEQ (<=, >=), never ULT/UGT/SLT/SGT (<, >) -- Z3's AST uses
// distinct decl kinds for strict vs non-strict comparisons, and the original
// parser only recognizes the non-strict ones, silently dropping any strict
// inequality constraint from the encoding entirely. The trained weights were
// fit against that exact (buggy) distribution, so a "corrected" encoder here
// would feed the model out-of-distribution input and likely perform worse,
// not better, without retraining. Ported bug-for-bug on purpose.
static const int GAN_MAX_VARS = 64;
static const int GAN_MAX_CONSTRAINTS = 128;
static const int GAN_VAR_FEAT = 4;
static const int GAN_OP_DIM = 16;

static int ganFeatureDim() {
    return GAN_MAX_VARS * GAN_VAR_FEAT + GAN_MAX_CONSTRAINTS * (GAN_MAX_VARS + GAN_OP_DIM + 2);
}

// Only the ops the original _BV_OP_INDEX actually recognizes. Index values
// match bv_encoder.py's _BV_OP_INDEX dict exactly (not its own docstring,
// which is stale/wrong about which index means what -- the dict is what the
// model was actually trained against).
static int ganOpIndex(const std::string &op) {
    if (op == "bvule") return 0;
    if (op == "bvuge") return 2;
    if (op == "bvsle") return 5;
    if (op == "bvsge") return 7;
    if (op == "=") return 8;
    if (op == "distinct") return 9;
    if (op == "bvand") return 10;
    if (op == "bvor") return 11;
    if (op == "bvxor") return 12;
    if (op == "bvadd") return 13;
    if (op == "bvsub") return 14;
    if (op == "bvmul") return 15;
    return -1; // includes bvult/bvugt/bvslt/bvsgt -- intentionally unmapped
}

static void ganCollectVars(std::vector<Node> &nodes, int nid,
                            std::unordered_map<int,int> &varIndex,
                            std::vector<float> &mask) {
    if (nid < 0 || (size_t)nid >= nodes.size()) return;
    Node &n = nodes[nid];
    if (n.op == "var") {
        auto it = varIndex.find(nid);
        if (it != varIndex.end() && it->second < GAN_MAX_VARS) mask[it->second] = 1.0f;
        return;
    }
    for (int a : n.args) ganCollectVars(nodes, a, varIndex, mask);
}

static float ganLiteralNorm(std::vector<Node> &nodes, int nid) {
    if (nid < 0 || (size_t)nid >= nodes.size()) return 0.0f;
    Node &n = nodes[nid];
    if (n.op != "bvlit") return 0.0f;
    int width = n.width;
    if (width <= 0) return 0.0f;
    double maxv = (double)((1ULL << width) - 1ULL);
    if (maxv <= 0) return 0.0f;
    return (float)((double)(uint64_t)n.value / maxv);
}

// Flatten top-level ANDs only (matching the Python encoder's own recursion,
// which only special-cases Z3_OP_AND -- NOT does not get flattened, so any
// constraint arriving wrapped in NOT is silently dropped too, same as the
// strict-inequality gap above).
static void ganWalkConstraints(std::vector<Node> &nodes, int nid,
                                std::unordered_map<int,int> &varIndex,
                                std::vector<int> &out) {
    if (nid < 0 || (size_t)nid >= nodes.size()) return;
    Node &n = nodes[nid];
    if (n.op == "and") {
        for (int a : n.args) ganWalkConstraints(nodes, a, varIndex, out);
        return;
    }
    if (ganOpIndex(n.op) >= 0 && n.args.size() >= 2) out.push_back(nid);
}

// Encodes ir's CURRENT assertions (call this right after parsing, before any
// of our own preprocessing passes mutate the graph, to match what the
// Python path would have seen: the raw parsed formula).
static torch::Tensor ganEncode(IR &ir) {
    std::vector<float> vec(ganFeatureDim(), 0.0f);

    std::unordered_map<int,int> varIndex; // node id -> slot index
    std::unordered_map<std::string,int> nameToNode;
    for (size_t i = 0; i < ir.nodes.size(); i++)
        if (ir.nodes[i].op == "var") nameToNode[ir.nodes[i].name] = (int)i;

    int slot = 0;
    for (auto &d : ir.declares) {
        if (slot >= GAN_MAX_VARS) break;
        if (d.isBool) continue; // encoder only handles BV vars
        auto it = nameToNode.find(d.name);
        if (it == nameToNode.end()) continue;
        varIndex[it->second] = slot;
        int base = slot * GAN_VAR_FEAT;
        vec[base + 0] = (float)d.width / 64.0f;
        vec[base + 1] = 0.0f;
        vec[base + 2] = 1.0f;
        vec[base + 3] = 0.0f;
        slot++;
    ...
    for (int aid : ir.assertions) ganWalkConstraints(ir.nodes, aid, varIndex, constraints);

    int baseOffset = GAN_MAX_VARS * GAN_VAR_FEAT;
    int rowSize = GAN_MAX_VARS + GAN_OP_DIM + 2;
    int ci = 0;
    for (int nid : constraints) {
        if (ci >= GAN_MAX_CONSTRAINTS) break;
        Node &n = ir.nodes[nid];
        int opIdx = ganOpIndex(n.op);
        int offset = baseOffset + ci * rowSize;

        std::vector<float> mask(GAN_MAX_VARS, 0.0f);
        ganCollectVars(ir.nodes, n.args[0], varIndex, mask);
        ganCollectVars(ir.nodes, n.args[1], varIndex, mask);
        for (int k = 0; k < GAN_MAX_VARS; k++) vec[offset + k] = mask[k];
        vec[offset + GAN_MAX_VARS + opIdx] = 1.0f;
        vec[offset + GAN_MAX_VARS + GAN_OP_DIM]     = ganLiteralNorm(ir.nodes, n.args[0]);
        vec[offset + GAN_MAX_VARS + GAN_OP_DIM + 1] = ganLiteralNorm(ir.nodes, n.args[1]);
        ci++;
    }

    torch::Tensor t = torch::from_blob(vec.data(), {1, ganFeatureDim()}, torch::kFloat32).clone();
    return t;
}

// [-1,1]^64 generator output -> {node id (of the 'var' node) : integer value}
static std::unordered_map<int, uint64_t> ganDecode(torch::Tensor out, IR &ir,
                                                     std::unordered_map<int,int> &varIndex) {
    std::unordered_map<int, uint64_t> result;
    auto acc = out.accessor<float,2>();
    // invert varIndex: slot -> node id
    std::unordered_map<int,int> slotToNode;
    for (auto &kv : varIndex) slotToNode[kv.second] = kv.first;
    for (auto &kv : slotToNode) {
        int slotI = kv.first, nid = kv.second;
        int width = ir.nodes[nid].width;
        if (width <= 0) continue;
        double maxv = (double)((1ULL << width) - 1ULL);
        float raw = acc[0][slotI];
        double value = std::round(((double)raw + 1.0) / 2.0 * maxv);
        if (value < 0) value = 0;
        if (value > maxv) value = maxv;
        result[nid] = (uint64_t)value;
    }
    return result;
}
...
static int run_solver(int argc, char **argv) {
    ...
    if (std::getenv("GAN_TEST")) {
        static torch::jit::script::Module ganModel;
        static bool ganLoaded = false;
        if (!ganLoaded) {
            ganModel = torch::jit::load("/tmp/gansat_bv_traced.pt");
            ganModel.eval();
            ganLoaded = true;
        }
        std::unordered_map<int,int> varIndex;
        ...
        int slot = 0;
        for (auto &d : ir.declares) {
            if (slot >= GAN_MAX_VARS || d.isBool) continue;
            auto it = nameToNode.find(d.name);
            if (it == nameToNode.end()) continue;
            varIndex[it->second] = slot;
            slot++;
        ...
        torch::Tensor f = ganEncode(ir);
        torch::NoGradGuard no_grad;
        torch::Tensor z = torch::randn({1, 128});
        std::vector<torch::jit::IValue> inputs{f, z};
        auto t0 = std::chrono::steady_clock::now();
        torch::Tensor out = ganModel.forward(inputs).toTensor();
        auto t1 = std::chrono::steady_clock::now();
        auto assignment = ganDecode(out, ir, varIndex);

        fprintf(stderr, "[GAN-TEST] encode+inference time: %.3f ms\n",
                std::chrono::duration<double, std::milli>(t1 - t0).count());
        fprintf(stderr, "[GAN-TEST] candidate assignment (%zu vars):\n", assignment.size());
        for (auto &kv : assignment)
            fprintf(stderr, "  %s = %llu\n", ir.nodes[kv.first].name.c_str(), (unsigned long long)kv.second);

        // Cheap verification: substitute the candidate into every top-level
        // assertion and check with plain integer arithmetic whether it
        // actually satisfies the formula -- this is the "is the guess any
        // good" check, done without touching MiniSat at all.
        std::function<int64_t(int)> evalBV = [&](int nid) -> int64_t {
            Node &n = ir.nodes[nid];
            if (n.op == "bvlit") return (int64_t)(uint64_t)n.value;
            if (n.op == "var") {
                auto it = assignment.find(nid);
                return it != assignment.end() ? (int64_t)it->second : 0;
            }
            if (n.op == "bvadd") return evalBV(n.args[0]) + evalBV(n.args[1]);
            if (n.op == "bvsub") return evalBV(n.args[0]) - evalBV(n.args[1]);
            if (n.op == "bvmul") return evalBV(n.args[0]) * evalBV(n.args[1]);
            return 0;
        };
        std::function<bool(int)> evalBool = [&](int nid) -> bool {
            Node &n = ir.nodes[nid];
            if (n.op == "and") { for (int a : n.args) if (!evalBool(a)) return false; return true; }
            if (n.op == "=") return evalBV(n.args[0]) == evalBV(n.args[1]);
            if (n.op == "distinct") return evalBV(n.args[0]) != evalBV(n.args[1]);
            if (n.op == "bvule") return (uint64_t)evalBV(n.args[0]) <= (uint64_t)evalBV(n.args[1]);
            if (n.op == "bvuge") return (uint64_t)evalBV(n.args[0]) >= (uint64_t)evalBV(n.args[1]);
            if (n.op == "bvult") return (uint64_t)evalBV(n.args[0]) < (uint64_t)evalBV(n.args[1]);
            if (n.op == "bvugt") return (uint64_t)evalBV(n.args[0]) > (uint64_t)evalBV(n.args[1]);
            return false; // unknown/unsupported op in this quick check -> treat as unverified
        };
        int total = 0, verified = 0;
        for (int aid : ir.assertions) {
            total++;
            try { if (evalBool(aid)) verified++; } catch (...) {}
        }
        fprintf(stderr, "[GAN-TEST] assertions verified by candidate: %d / %d\n", verified, total);
    }
