...
behind Bitwuzla (2024--2025 winner, 938.9\,ms) — while solving \emph{one more}
...
Bitwuzla (2024--2025 winner) & 90 & 10 & 938.9\,ms & — \\
Z3                     & 66 & 34 &  3{,}192\,ms$^*$ & — \\
\bottomrule
...
% ─── 5. USAGE ────────────────────────────────────────────────────────────────
\section{Running NeuroSym}

NeuroSym follows the standard SMT-COMP single-query interface.
The solver receives one benchmark file as a command-line argument and
writes \texttt{sat}, \texttt{unsat}, or \texttt{unknown} to stdout.

\begin{verbatim}
# Single benchmark
NeuroSym-1.1/python3 benchmark.smt2

# With explicit timeout (seconds)
NeuroSym-1.1/python3 --timeout 20 benchmark.smt2
\end{verbatim}

\noindent\textbf{Dependencies} (bundled in \texttt{lib/}):
z3-solver\,$\geq$\,4.12, bitwuzla\,$\geq$\,0.9, numpy\,$\geq$\,1.24,
networkx\,$\geq$\,3.1, pysmt\,$\geq$\,0.9.5.
PyTorch is \emph{optional}; without it the solver runs the symbolic
fallback only (Bitwuzla for BV, Z3 for LIA).

\noindent\textbf{Supported logics:} QF\_LIA, QF\_BV, QF\_ABV.
...
\begin{itemize}
  ...
  \item \textbf{PAR-2:} 949\,ms vs.\ Bitwuzla 939\,ms — within 1.1\%
        of the 2024--2025 competition winner.
